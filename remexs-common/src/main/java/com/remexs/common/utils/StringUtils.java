...
public class StringUtils extends org.apache.commons.lang3.StringUtils{
	...
	public static String blob2String(Blob blob) {
		if (null != blob) {
			try {
				...
				throw new ServiceException("Blob Convert To String Error!");
			}
		}
		return null;
	}
	 ...
	 * @return
	 */
	public static String merge(String content, Object... args) {
		...
		if (args != null) {
			...
			if (length >= 1) {
				for (int i = 0; i < length; i++) {
					content = content.replace(String.format(PLACE_HOLDER, i), args[i].toString());
				}
			}
		}
		return content;
	 ...
	 * @return
	 */
	public static String mergeSql(String content, Object... args) {
		if (StringUtils.isEmpty(content)) {
			return null;
		}
		if (args != null) {
			...
			if (length >= 1) {
				for (int i = 0; i < length; i++) {
					content = content.replace(String.format(PLACE_HOLDER, i), quotaMark(args[i]));
				}
			}
		}
		return content;
	...
	private static String wordsAndHyphenAndCamelToConstantCase(String input) {
		...
		for (int i = 0; i < chars.length; i++) {
			...
			boolean lastOneIsNotUnderscommon = (buf.length() > 0) && (buf.charAt(buf.length() - 1) != '_');
			boolean isNotUnderscommon = c != '_';
			if ((lastOneIsNotUnderscommon) && ((isUpperCaseAndPreviousIsLowerCase) || (previousIsWhitespace) || ((betweenUpperCases)
			...
			if ((shouldReplace(c)) && (lastOneIsNotUnderscommon)) {
				buf.append('_');
			} else if ((!Character.isWhitespace(c)) && ((isNotUnderscommon) || (lastOneIsNotUnderscommon))) {
